...
void CLegitBot::DoTrigger(CUserCmd *pCmd)
{
	...
	if (Trace.m_pEnt && 0 < Trace.hitgroup <= 7) // hitbox not hitgroup
	{
		...
		{
			TriggerTime2 = timeGetTime();
			TriggerTime2 = TriggerTime2 - TriggerTime;
			float delay = Menu::Window.LegitBotTab.TriggerDelay.GetValue() / 100.f;

			if (TriggerTime2 < delay)
			{
				return;
			}
			else
